...
app.post('/upload', (req, res) => {
  res.setTimeout(5000, function(){
    console.log('Request timed out.');
      req.session.message = 'Request timed out';
      res.send(408);
    });
...
function ffmpegStart(myID,meme,audioPath,gifPath,videoPath) {
  ( async () => {
    ...
    if (meme === 'true') {
      ...
      command = ['-y', '-i', audioPath, '-ignore_loop', '0', '-i',
      gifPath, '-vf', "scale=720:trunc(ow/a/2)*2", '-shortest', '-strict', '-2', '-c:v', 'libx264', '-threads', '12', '-c:a', 'aac', '-b:a', '320k', '-q:a', '100', '-filter_complex', 'alimiter=level_in=1:level_out=1:limit=1:attack=1:release=100:level=disabled,asetrate=44100*1.0,aresample=44100', '-pix_fmt', 'yuv420p', '-crf', '28', '-preset', 'faster', '-profile:v', 'baseline', videoPath];
    }    
